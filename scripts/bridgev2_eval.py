    ...
    for idx, example in enumerate(tqdm.tqdm(ds, total=total_examples)):
        ...
        if gt_actions is not None and gt_actions.size > 0:
            ...
            if aligned > 0:
                ...
                head = min(5, aligned)
                print("  GT first actions:", gt_actions[:head])
                print("  Pred mean first actions:", pred_mean[:head])
                if predicted_actions.shape[1] > 1:
                    print("  Pred sample[0] first actions:", predicted_actions[:head, 0])
                    print("  Pred std first actions:", predicted_actions[:head].std(axis=1))
            else:
                print("  No overlapping steps between ground truth and predictions.")
        else:
            print(
